...
if not os.path.exists('plots_d18o_errorbars'):
    os.makedirs('plots_d18o_errorbars')
...
#search for newest_isotope_data (chosen with the same criterion as the age model): 
      ...
      if len(e) > 1 & e['d18O (VPDB)'].isnull().all()==0.00:
       taxon = e.iloc[0,2]
...
#remove lines with repeating numbers in the age model:
        ...
        depthageisotope = depthageisotope[depthageisotope['d18O (VPDB)'].notna()]
        d18o = depthageisotope['d18O (VPDB)']
        ...
        plt.plot(age,d18o,label=agemodelnote, marker="o",ms=5,color = colors[counter])
        plt.errorbar(age,d18o, xerr = asymmetric_error, fmt = 'o',ecolor = colors[counter], elinewidth = 2, capsize=3, ms=0.000001, alpha=0.5)
        plt.legend()
        plt.tick_params(which='major', labelsize=8, labelbottom=True)
        plt.xlabel('time (y BP)',fontsize=15)
        plt.ylabel('$\delta^{18}O\;\;(^o/_{oo})$',fontsize=15)
        plt.xlim([max(10000,min(age)),min(23000,max(age))])
        plt.ylim([min(d18o)-0.2,max(d18o)+0.2])       
        plt.title(sitename + ',  ' + taxon,fontsize=14)
        plt.savefig('plots_d18o_errorbars/' + sitename + '.pdf', format='pdf', dpi=5000)
        counter = counter+1
       plt.plot().clear()
       plt.close()
       plt.cla()
